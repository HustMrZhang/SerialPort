/*
 * datamaker.h
 *
 *  Created on: May 23, 2018
 *      Author: leon
 */

#ifndef DATAMAKER_H_
#define DATAMAKER_H_

#include <stdio.h>
#include "serial.h"
#include <deque>
#include "AOAReciver.h"

class DataMaker {
public:
	Serial* m_serial;
	bool m_read_thread_alive;
private:
	pthread_t m_rec_threadid;
	int m_serial_fd;
	std::deque<unsigned char> frames;
	std::deque<unsigned char> datas;
	AOAReciver * m_listener;
public:
	DataMaker(AOAReciver *listener);
	virtual ~DataMaker();
	void run();
	int startWork(int index);
	int stop();
	int checkCrc16(std::deque<unsigned char> *pucFrame);

private:
	void analysisData();
	void publishData();
	void convertDequeToArrays(unsigned char *data,
			std::deque<unsigned char> *pucFrame);
};

static const unsigned char aucCRCHi[] = {  };
static const unsigned char aucCRCLo[] = {  };

unsigned short int Get_Crc16(unsigned char *pucFrame, unsigned short int usLen);
void clear(unsigned char * data, int length);
#endif /* DATAMAKER_H_ */
